...
DWORD XeKeysUnObfuscateHook(DWORD r3, BYTE* r4, DWORD r5, BYTE* r6, DWORD r7)
{
	...
	/*
		...
		r7 - ?
	*/
	XEKEYSUNOBFUSCATE XeKeysUnObfuscate = (XEKEYSUNOBFUSCATE)ResolveFunction("xboxkrnl.exe", 597);
	if (r3 == 1) // From tests I did, profiles only called when set to 1
	{
		...
		else // fail: swap key and try again
		{
			...
			XeCryptHmacSha((const PBYTE)HmacShaKey, 0x10, hash, 0x10, 0, 0, 0, 0, rc4Key, 0x10);
			XeCryptRc4Key(&rc4State, rc4Key, 0x10);
			XeCryptRc4Ecb(&rc4State, hash + 0x10, 0x8);
			XeCryptRc4Ecb(&rc4State, (BYTE *)r6, r5 - 0x18);
			XeCryptHmacSha((const PBYTE)HmacShaKey, 0x10, hash+0x10, 8, (const PBYTE)r6, r5 - 0x18, 0, 0, newHash, 0x10);
...
{
    XEKEYSOBFUSCATE XeKeysObfuscate = (XEKEYSOBFUSCATE)ResolveFunction("xboxkrnl.exe", 596);
    if (r3 == 1)
    {
        ...
        *r7 = r5 + 0x18;
        ...
        // Encrypt the data
        XeCryptRc4((const PBYTE)rc4Key, 0x10, r6 + 0x10, *r7 - 0x10);
        return 0;
    }
    else
    {
