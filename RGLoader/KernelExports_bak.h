...
EXTERN_C {
	...
	void KeQuerySystemTime(QWORD* Time);
	...
	NTSTATUS XeKeysGetConsoleID(DWORD ZeroNumber, char* KeyBuffer);
	...
	HRESULT ObLookupAnyThreadByThreadId(DWORD dwThreadId, PHANDLE pHandle);
	
	VOID ObDereferenceObject(PVOID Object);
	NTSTATUS ObReferenceObjectByHandle(HANDLE Handle, DWORD ObjectType, PVOID *Object);

	DWORD KeRaiseIrqlToDpcLevel();
	VOID KfLowerIrql(DWORD Irql);
	VOID KeEnterCriticalRegion();
	VOID KeLeaveCriticalRegion();
	DWORD KfAcquireSpinLock(LPVOID Object);
	VOID KfReleaseSpinLock(LPVOID Object, DWORD Irql);
	...
	BOOL ExTerminateThread(HANDLE hThread);

	BOOL KeSetEvent(HANDLE Event, DWORD Set, DWORD Unk);
	VOID KeResetEvent(HANDLE Event);
	VOID KeSetBasePriorityThread(PVOID Thread, DWORD Priority);
